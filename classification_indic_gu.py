...
TRAIN_DATAPATH = f'/raid/nlp/pranavg/meet/IndicLLM/IndicGPT/data/train-data-ft-gu-{TASK_NAME}/train.pt'
VAL_DATAPATH = f'/raid/nlp/pranavg/meet/IndicLLM/IndicGPT/data/val-data-ft-gu-{TASK_NAME}/val.pt'
...
classificationhead.to('cuda')

optimizer = optim.AdamW(list(model.parameters())+list(classificationhead.parameters()), lr=1e-5)
...
for j in tqdm(range(0,val_data.shape[0],BATCH_SIZE)):
    with torch.no_grad():
        ...
        classification_out = classificationhead(last_states)
        ...
        if NUM_CLASSES>1:
            ...
            preds = torch.round(classification_out).squeeze()
        #print(preds.shape,enc_labels.shape)
        assert preds.shape == enc_labels.shape 
        corr += torch.sum(preds==enc_labels)
        labels.extend(preds.tolist())
        total += preds.shape[0]
print(f'Labels = {sum(labels)/len(labels)}')
print(f'Val accuracy: {corr/total}')
print(f'MCC = {matthews_corr(torch.tensor(labels),val_data[:,-1].squeeze())}')
print(f'F1 = {f1(torch.tensor(labels),val_data[:,-1].squeeze())}')
...
for epoch in tqdm(range(EPOCHS)):
    ...
    for i in tqdm(range(0,train_data.shape[0],BATCH_SIZE)):
        ...
        enc_text = enc_text.long()
        enc_text = enc_text.to('cuda')
        enc_labels = enc_labels.long()
        enc_labels = enc_labels.to('cuda')
        logits,states = model(enc_text)
        last_states = get_indexes(enc_text, states)
        #last_states = states[:,-1,:]
        #del enc_text, states,logits
        classification_out = classificationhead(last_states).squeeze()
...
torch.save(classificationhead,checkpoint_dir+'head-last.ckpt')
