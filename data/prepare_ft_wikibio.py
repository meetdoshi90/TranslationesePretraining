...
for row in data['train']:
    #print(row)
    train.append(row['serialized_infobox'])
    train_mask.append(row['summary'])
    #break
...
i=0
for j in tqdm(range(0,train.shape[0],500)):
    i+=1
    t = train[j:j+500,:].clone()
    tm = train_mask[j:j+500,:].clone()
